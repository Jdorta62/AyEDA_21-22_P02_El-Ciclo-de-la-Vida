...
int StateDead::Neighbors(Grid const &grid, int i, int j) {
  ...
  int number_of_neighbors_alive_state_counter{0};
  for(int k{i-1}; k <= i + 1; ++k) {
    for(int l{j-1}; l <= j + 1; ++l) {
      if (k > 0 && l > 0 && k < grid.GetRows() - 1 && l < grid.GetColumns() - 1 && (k != i || l != j)) {
        Cell temp_cell = grid.GetCell(k,l);
        if(temp_cell.GetState()->GetState() != 'D' && (k == i || l == j)) {
          ++number_of_neighbors_alive_state_counter;
        }
      }
    }
  }
  number_of_neighbors_alive_state_ = number_of_neighbors_alive_state_counter;
  //std::cout << "Soy del estado muerta. " << number_of_neighbors_adult_state_ << std::endl;
  return number_of_neighbors_alive_state_;
}
...
State* StateDead::NextState() {
  ...
  if (number_of_neighbors_alive_state_ == 2) {
